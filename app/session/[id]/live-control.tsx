...
// ─────────────────────────────────────────────
// Animação de Fundo (Ajustada para o Telão)
// ─────────────────────────────────────────────
function NetworkBackground() {
  ...
  useEffect(() => {
    const initParticles = Array.from({ length: 40 }).map(() => ({
      ...
      vx: (Math.random() - 0.5) * 0.4, // Mais lento para não distrair
  ...
  for (let i = 0; i < particles.length; i++) {
    for (let j = i + 1; j < particles.length; j++) {
      ...
      if (dist < 180) {
        const opacity = (1 - dist / 180) * 0.15; // Bem mais transparente
        lines.push(
  ...
  return (
    <View style={StyleSheet.absoluteFillObject} pointerEvents="none">
      {/* Dois "Glows" radiais nos cantos para dar clima */}
...
export default function LiveControlScreen() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(sessionRef, (snapshot) => {
      ...
      if (data) {
        ...
        let total = 0;
        if (responses) {
          Object.values(responses).forEach((val: any) => { counts[val] = (counts[val] || 0) + 1; total++; });
        }
        setVotes(counts);
        setTotalVotes(total);
      } else {
  ...
  if (loading || errorMessage) return <LoadingOrError loading={loading} errorMessage={errorMessage} />;
  ...
  return (
    <View style={styles.root}>
      ...
      <View style={styles.topBar}>
        ...
        {/* Cartão Central Elegante do PIN */}
...
// Helpers
// ─────────────────────────────────────────────
function LoadingOrError({ loading, errorMessage }: { loading: boolean, errorMessage: string }) {
  return (
...
const styles = StyleSheet.create({
  ...
  bgGlow: { position: 'absolute', width: 600, height: 600, borderRadius: 300, filter: 'blur(100px)' as any }, // Glow effect for web
  ...
  topBar: { 
    ...
    backdropFilter: 'blur(10px)' as any // Blur for web
  },
  ...
  qrGlassCard: {
    backgroundColor: 'rgba(26, 25, 36, 0.6)',
    padding: 32,
    borderRadius: 40,
    borderWidth: 1,
