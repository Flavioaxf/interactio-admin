...
export default function DashboardScreen() {
  ...
  return (
    <View style={[styles.root, { flexDirection: isMobile ? 'column' : 'row' }]}>
      ...
      <View style={[styles.mainArea, isMobile && { padding: 20 }]}>
        ...
        {loadingSessions ? (
          ...
          <View style={[styles.emptyStateContainer, isMobile && { padding: 20 }]}>
            ...
            <Text style={[styles.emptyStateTitle, isMobile && { fontSize: 20, textAlign: 'center' }]}>Nenhuma sessão recente</Text>
            <Text style={styles.emptyStateDesc}>Crie sua primeira sessão interativa para engajar o seu público em tempo real.</Text>
          </View>
        ) : (
                  ...
                  <View key={session.id} style={[styles.sessionCard, { width: cardWidth as any }]}>
                    <View style={styles.cardHeader}>
                      <View style={[styles.statusBadge, isDraft ? styles.statusDraft : styles.statusActive]}>
                        ...
                        <Text style={[styles.statusText, { color: isDraft ? '#8b89a0' : '#34d399' }]}>{isDraft ? 'Rascunho' : 'Ao Vivo'}</Text>
                      </View>
                    ...
                    <View style={styles.cardActions}>
                      ...
                      {!isMobile && (
                        <TouchableOpacity style={styles.cardBtnPrimary} onPress={() => router.push(`/session/${session.id}/live-control` as any)} activeOpacity={0.8}>
      ...
      {/* ── MODAL NOVA SESSÃO CORRIGIDO PARA MOBILE ── */}
      {isModalVisible && !isLoggingOut && !creationState.active && !sessionToDelete && (
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, isMobile && { width: '90%', padding: 24, maxHeight: '85%' }]}>
            ...
            {/* O ScrollView flexShrink permite que role, mas não esmague os itens */}
            <ScrollView style={{ flexShrink: 1 }} showsVerticalScrollIndicator={false}>
              <View style={[styles.modeCardsContainer, isMobile && { flexDirection: 'column', gap: 16 }]}>
                
                <TouchableOpacity 
                  style={[styles.modeCard, !isMobile && { flex: 1 }, isMobile && { padding: 20, opacity: 0.5, borderColor: 'rgba(255,255,255,0.02)' }]} 
                  activeOpacity={0.8} 
                  onPress={() => !isMobile && handleSelectMode('live')}
                  disabled={isMobile}
                >
                  <View style={[styles.modeIconBox, { backgroundColor: isMobile ? 'rgba(255,255,255,0.05)' : 'rgba(56, 189, 248, 0.1)' }]}>
                    <Ionicons name={isMobile ? "lock-closed" : "flash"} size={32} color={isMobile ? "#5a5872" : "#38bdf8"} />
                  </View>
                  <Text style={[styles.modeTitle, isMobile && { color: '#8b89a0' }]}>Modo Rápido</Text>
                  
                  {isMobile ? (
                    <Text style={[styles.modeDesc, { color: '#ef4444', fontWeight: 'bold', marginBottom: 16 }]}>Disponível apenas no computador.</Text>
                  ) : (
                    <Text style={styles.modeDesc}>Crie e lance uma interação instantaneamente.</Text>
                  )}
                  
                  {!isMobile && (
                    <View style={styles.modeAction}><Text style={[styles.modeActionText, { color: '#38bdf8' }]}>Lançar agora →</Text></View>
                  )}
                </TouchableOpacity>

                <TouchableOpacity 
                  style={[styles.modeCard, !isMobile && { flex: 1 }, isMobile && { padding: 20 }]} 
                  activeOpacity={0.8} 
      ...
      {creationState.active && (
        <View style={styles.overlayScreen}>
          <View style={styles.overlayContent}>
            <View style={[styles.overlayIconWrapper, { backgroundColor: creationState.mode === 'live' ? 'rgba(56, 189, 248, 0.1)' : 'rgba(167, 139, 250, 0.1)', borderColor: creationState.mode === 'live' ? 'rgba(56, 189, 248, 0.2)' : 'rgba(167, 139, 250, 0.2)' }]}>
...
const styles = StyleSheet.create({
  ...
  mobileHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 20, paddingVertical: 16, backgroundColor: '#13121d', borderBottomWidth: 1, borderBottomColor: 'rgba(255,255,255,0.05)' }, mobileLogoutBtn: { padding: 8, backgroundColor: 'rgba(239, 68, 68, 0.1)', borderRadius: 12 },
  sidebar: { width: 280, backgroundColor: '#13121d', padding: 24, borderRightWidth: 1, borderRightColor: 'rgba(255,255,255,0.05)', justifyContent: 'space-between' }, logoContainer: { marginBottom: 40 }, logoText: { fontSize: 28, fontWeight: '900', color: '#e8e6f0', letterSpacing: -1 }, highlightText: { color: '#a78bfa' }, roleBadge: { backgroundColor: 'rgba(167, 139, 250, 0.1)', alignSelf: 'flex-start', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 6, marginTop: 8 }, roleText: { color: '#a78bfa', fontSize: 10, fontWeight: '800', letterSpacing: 1 }, navMenu: { flex: 1, gap: 8 }, navItem: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 14, paddingHorizontal: 16, borderRadius: 12 }, navItemActive: { backgroundColor: 'rgba(167, 139, 250, 0.1)' }, navText: { color: '#8b89a0', fontSize: 15, fontWeight: '600' }, navTextActive: { color: '#a78bfa', fontWeight: '800' }, logoutButton: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 14, paddingHorizontal: 16, opacity: 0.8 }, logoutText: { color: '#ef4444', fontSize: 15, fontWeight: '600' },
  ...
  modalHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 40 }, modalTitle: { color: '#e8e6f0', fontSize: 28, fontWeight: '900' }, closeModalBtn: { width: 40, height: 40, borderRadius: 20, backgroundColor: 'rgba(255,255,255,0.05)', justifyContent: 'center', alignItems: 'center' }, modeCardsContainer: { flexDirection: 'row', gap: 24 }, 
  
  // Retirei o flex: 1 daqui para não bugar no ScrollView do mobile
  modeCard: { backgroundColor: '#0f0e17', borderRadius: 24, padding: 32, borderWidth: 1, borderColor: 'rgba(255,255,255,0.05)' }, 
  
  modeIconBox: { width: 64, height: 64, borderRadius: 16, justifyContent: 'center', alignItems: 'center', marginBottom: 24 }, modeTitle: { color: '#e8e6f0', fontSize: 24, fontWeight: '800', marginBottom: 12 }, 
  modeDesc: { color: '#8b89a0', fontSize: 15, lineHeight: 22, marginBottom: 32, flex: 1 }, 
  modeAction: { borderTopWidth: 1, borderTopColor: 'rgba(255,255,255,0.05)', paddingTop: 16 }, modeActionText: { fontSize: 15, fontWeight: '800' },
  ...
  deleteModalOverlay: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(15, 14, 23, 0.85)', justifyContent: 'center', alignItems: 'center', zIndex: 2000, ...(Platform.OS === 'web' && { backdropFilter: 'blur(8px)' } as any) }, deleteModalContent: { width: '100%', maxWidth: 450, backgroundColor: '#1a1924', borderRadius: 32, padding: 40, borderWidth: 1, borderColor: 'rgba(239, 68, 68, 0.2)', alignItems: 'center', shadowColor: '#ef4444', shadowOffset: { width: 0, height: 20 }, shadowOpacity: 0.15, shadowRadius: 40 }, deleteIconWrapper: { width: 80, height: 80, borderRadius: 40, backgroundColor: 'rgba(239, 68, 68, 0.1)', justifyContent: 'center', alignItems: 'center', marginBottom: 24, borderWidth: 1, borderColor: 'rgba(239, 68, 68, 0.2)' }, deleteModalTitle: { color: '#e8e6f0', fontSize: 24, fontWeight: '900', marginBottom: 12, textAlign: 'center' }, deleteModalDesc: { color: '#8b89a0', fontSize: 16, textAlign: 'center', lineHeight: 24 }, deleteButtonsRow: { flexDirection: 'row', gap: 16, width: '100%', marginTop: 32 }, deleteCancelBtn: { flex: 1, paddingVertical: 16, borderRadius: 16, backgroundColor: 'rgba(255,255,255,0.05)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)', alignItems: 'center' }, deleteCancelBtnText: { color: '#e8e6f0', fontSize: 15, fontWeight: '700' }, deleteConfirmBtn: { flex: 1, paddingVertical: 16, borderRadius: 16, backgroundColor: '#ef4444', alignItems: 'center', shadowColor: '#ef4444', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 10 }, deleteConfirmBtnText: { color: '#ffffff', fontSize: 15, fontWeight: '800' }
});